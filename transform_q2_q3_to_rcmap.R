...
# Transform Q2 and Q3 Qualtrics Data to RCMap Format (separate files)
...
cat("Reading cleaned Q2/Q3 data...\n")
data <- read_csv(input_file, show_col_types = FALSE)

# Identify Q2 and Q3 columns
col_names <- names(data)
q2_cols <- grep("^Q2_", col_names, value = TRUE)
q3_cols <- grep("^Q3_", col_names, value = TRUE)
meta_cols <- setdiff(col_names, c(q2_cols, q3_cols))
...
ratings_q2 <- data %>%
  ...
  mutate(
    StatementID = as.integer(str_extract(QCol, "\\d+")) + 1  # +1 for 1-based
  ) %>%
...
# --- Ratings_Q3.csv ---
cat("Creating Ratings_Q3.csv...\n")
ratings_q3 <- data %>%
  mutate(ParticipantID = row_number()) %>%
  select(ParticipantID, all_of(q3_cols)) %>%
  pivot_longer(
    cols = all_of(q3_cols),
    names_to = "QCol",
    values_to = "Rating"
  ...
  select(ParticipantID, StatementID, Rating) %>%
  filter(!is.na(Rating) & Rating != "" & Rating != "NA")
write_csv(ratings_q3, file.path(output_dir, "Ratings_Q3.csv"))
...
cat("You can now run RCMap as usual on Q2 or Q3 ratings.\n") 